...
?>
<h2>Chat with <?=$receiver->profile['firstname'] . " " . $receiver->profile['lastname'] ?></h2>
<?php
...
<div class="text-<?=$side?>">
	...
	<?php
		if(date('d-m-Y', time()) == date('d-m-Y', (int)$item['date_created'])){
			echo date('H:i:s', $item['date_created']);
		}else{
			echo date('d-m-Y H:i:s', (int)$item['date_created']);
		}
	 ?>
</div>
