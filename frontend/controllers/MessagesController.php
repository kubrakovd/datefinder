...
class MessagesController extends \yii\web\Controller
{
    ...
    public function actionView($id)
    {
    	...
    	if(isset($_POST['MessageForm']['message'])){
    		...
    		$message->from_user = (int)Yii::$app->user->id;
    		$message->date_created = (string)time();
    		$message->viewed = (int)'0';
    		$message->save();
    	}
        ...
        $receiver = User::find()->where(['id'=>$id])->one();
        return $this->render('view',['message_list'=>$message_list, 'model'=>$model, 'receiver'=>$receiver ]);
